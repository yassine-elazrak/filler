...
void		ft_index(t_init *lst_a, int *tab, int s_t)
{
	
	while (lst_a)
	{
		lst_a->pos = ft_search(tab, lst_a->data , 0,  s_t);
		lst_a = lst_a->next;
	}
}
void		ft_mangement_sort(t_init **lst_a, t_init **lst_b)
{
	...
	*lst_b = NULL;
	s_t = (*lst_a)->size;
	tab = ft_new_and_sort(*lst_a, s_t);
	//ft_print_tab(tab,s_t);
	ft_index((*lst_a), tab, s_t);
	//ft_print_tab(tab, s_t);
	//printf("a= %d\n b = %d\n",tab[s_t - 1], tab[0]);
	...
	i = 0;
		while (i < 200)
		{
			...
			ft_print(*lst_a,"list_a","00");
		 ft_print(*lst_b, "lst_b","00");
